...
QUnit.test("Array access into the path seg list", function(assert) {
    var path = document.createElementNS("http://www.w3.org/2000/svg", "path");

    path.setAttribute("d", "M50 51Z");
    assert.equal(path.pathSegList[0].toString(), "[object SVGPathSegMovetoAbs]");
    assert.equal(path.pathSegList[0].x, "50");
    assert.equal(path.pathSegList[0].y, "51");
});
